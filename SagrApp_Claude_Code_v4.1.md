...
Tutte le pagine della piattaforma hanno una **sidebar a sinistra** con la navigazione completa. È un componente condiviso (sidebar.js) incluso in ogni pagina.
...
| **ADMIN** 🔒 | Dashboard Live | /admin | Richiede PIN |
| | Dashboard Recap | /admin/recap | Richiede PIN |
| | Magazzino | /admin/magazzino | Richiede PIN |
| | Controllo Hardware | /admin/hardware | Richiede PIN |
| | Setup Turno | /setup | Richiede PIN |
| | Chiusura Turno | /admin/chiusura | Richiede PIN |

**Comportamento:**
- La voce della pagina attiva è **evidenziata** (sfondo diverso o bordo laterale colorato)
- Le voci ADMIN mostrano un lucchetto: se l'utente non è loggato, clic → redirect al login PIN
- La sidebar è **comprimibile**: pulsante hamburger (☰) per ridurla a sole icone o nasconderla
- Su **monitor cuochi e scaldavivande**: sidebar nascosta di default (schermo pieno per i dati), accessibile con tap sull'icona ☰ nell'angolo
- La sidebar mantiene lo stato (aperta/chiusa) in `localStorage`
- Tema scuro coerente con il resto della piattaforma
- Touch-friendly: voci alte almeno 44px per tap facile su tablet

**Implementazione:**
- File `sidebar.js` caricato da ogni pagina HTML
- Genera il DOM della sidebar dinamicamente
- Evidenzia la voce attiva in base all'URL corrente (`window.location.pathname`)
- Gestisce il PIN admin: se `sessionStorage` non ha il token, le voci admin fanno redirect a `/admin/login`
